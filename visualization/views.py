...
client = pymongo.MongoClient(
    "mongodb+srv://simplify:mineeproject@cluster0.glsg7.mongodb.net/?retryWrites=true&w=majority")
db = client.simplify
...
def vmshop(request):
  ...
  #  sales_by_product = data.groupby('Product')['Sales'].sum()
   ...
   # with loop:
    #    vmshop = partial(vmshop, request)
    #   loop.run_until_complete(asyncio.gather(vmshop()))
...
def vmowner(request):
    ...
    context = {'products': products,
               'shop_ids': shop_ids, 'plot_data': plot_data}
...
# Plotting the sales data using a line chart
    plt.plot(present_data['Product'],
             present_data['Sales'], label='Current Data')
    plt.plot(compare_data['Product'],
             compare_data['Sales'], label='Compare Data')
...
def outvmo(request):
    if request.method == 'POST':
      ...
      #  product_data = produc_data.iloc[:-12, :]
        ...
        months = pd.date_range(end=product_data['Date'].max(
        ), start=startdate, freq='MS').strftime('%B %Y')
        ...
        context = {'plot_data': plot_data, 'startdate': startdate, 'forecast': sales_dict,
...
def outvmpprodwise(request):
    ...
    if request.method == 'POST':
        ...
        context = {'plot_data': plot_data,
                   'forecast': forecast_dict, 'selected_shop': selected_shop}
...
def outvmoshop(request):
   # shop_id = request.user.shop_id
    ...
    #  sales_by_product = data.groupby('Product')['Sales'].sum()
        ...
        context = {'products': products,
                   'plot_data': plot_data, 'selected_shop': shop_id}
